// Aptidão para empreendimentos silviculturais de grande porte
// N5 
...
// Estradas
var distan_rod_fed = ee.Image("users/nepzapufvjm/bandas/dist_estrad_fed").clip(aoi).rename('distan_rod_fed');
var distan_rod_est = ee.Image("users/nepzapufvjm/bandas/dist_estrad_estadual").clip(aoi).rename('distan_rod_est');
var distan_rod_mun = ee.Image("users/nepzapufvjm/bandas/dist_estrad_municipal").clip(aoi).rename('distan_rod_mun');
...
//var referencias = ee.FeatureCollection("users/nepzapufvjm/expedicao");
//var referencias = ee.FeatureCollection("users/nepzapufvjm/expedicao-campo");
var referencias = ee.FeatureCollection("users/nepzapufvjm/expedicao-campo-imagem");
print(referencias);
...
var treinamento = layersExplanatory.select(bandas).sampleRegions({
   ...
   properties:['sg'], //seleciona coluna com valores 0 ou 1 para treinamento
   scale: 10
});
...
// Exportar
//Export.image.toDrive({ image: classificacaoComFiltro.clip(aoi), description: 'n4_g_silv', 
...
//Export.table.toDrive({collection: variable_importance01, description: 'n4_g_silv', fileFormat: 'CSV', folder: 'fapemig_mapeamento'});
...
var trainAccuracy = classificador.confusionMatrix();
print('Resubstitution error matrix: ', trainAccuracy);
print('Training overall accuracy: ', trainAccuracy.accuracy());
print('Índice Kappa RF', classificador.confusionMatrix().kappa());
print('Precisão Usuário RF', classificador.confusionMatrix().consumersAccuracy());
print('Precisão Produtor RF', classificador.confusionMatrix().producersAccuracy());